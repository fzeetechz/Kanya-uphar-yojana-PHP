...
<div class="card">
   ...
   <div class="card-body">
      <form action="{{ route('admin.pages.gallery.gallery-photos.update',$id) }}" enctype="multipart/form-data" method="POST" id="formid" name="formid">
         {{ csrf_field() }}
     ...
     <div class="form-group row">
            ...
            <label></label>
            @if($updatedata!='')
            <img src="{{getFile('gallery_photos',$updatedata->image)}}" class="img-thumbnail" width="80">
            @endif
            </div> 
      </div>
                  ...
                  <table id="dataTable" class="table table-striped table-bordered table-hover">
                     ...
                     <tbody>
                        ...
                        <tr class="notification{{$value->id}}">
                           ...
                           <td>{{$value->heading }}</td>
                           <td><img src="{{getFile('gallery_photos',$value->image)}}" class="img-thumbnail" width="80"></td>
                           <td>
                              <span class="Statuschange badge @if($value->status=='0') badge-danger @else badge badge-success @endif" data-id="{{$value->id}}"  data-model="GalleryPhoto" id="Statuschange{{$value->id}}">
                              @if($value->status=="0") Deactive @else Active @endif 
                              </span>
                           </td>
                           <td class="action ">
                              <a class="icon-btn delete universaldelete" href="javascript:void(0);" data-status="0" data-id="{{ $value->id}}"  data-model="GalleryPhoto" id="notification{{$value->id}}" > <i class="fal fa-trash-alt" id="delete-btn"></i></a>
                              <a class="icon-btn edit" href="{{route('admin.page.gallery.pages',['pagename'=>'gallery-photos','id'=>$value->id])}}"><i class="fal fa-edit" id="show-edit"></i></a>
                           </td>
                        </tr>
                        @endforeach
                     </tbody>
