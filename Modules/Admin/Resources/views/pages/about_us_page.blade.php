   ...
   <div class="card-body">
      <form action="{{ route('admin.pages.about-us.update',1) }}" method="POST" id="formid" enctype="multipart/form-data" name="formid">
         ...
         <div class="form-group row">
            <div class="col-md-8">
               <label for="nf-name">Section image</label>
               <input type="file" name="section_image" class="form-control">
               @if ($errors->has('section_image'))
               <span class="invalid feedback" role="alert"> 
               <strong class="text-danger">{{ $errors->first('section_image') }}.</strong> 
               </span>
               @endif
            </div>
            @if($updatedata!='') 
            <div class="col-md-4">
               <label></label>
               <img src="{{getFile('about_us_images',$updatedata->section_image)}}" class="img-thumbnail" width="80" onerror="this.src='/images/frontpages/aboutusimages/dummyimglawwa.jpg'">
            </div>
            @endif
         </div>
   </div>
