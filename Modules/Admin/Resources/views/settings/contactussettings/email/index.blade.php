...
<div class="card">
  <div class="card-header">
    <strong>Fill Email id Details</strong>
<a class="btn btn-sm btn-info float-right rounded-pill ml-5 ml-lg-2" href="{{ route('settings.contactussettings.email') }}" > <i class="fas fa-file-alt"></i> Emails</a>
   ...
   <div class="card-body">
      <form action="{{ route('settings.contactussettings.email.store',$id) }}" method="POST" id="formid" name="formid">
         {{ csrf_field() }}
         ...
         <div class="form-group">
            ...
            <input class="form-control {{ $errors->has('title') ? ' is-invalid' : '' }}" autocomplete="random" value="@if($updatedata!=''){{$updatedata->title}}@endif"  id="nf-title" name="title" type="text" 
               placeholder="Enter mail id title.." autocomplete="title">
                @if ($errors->has('title'))
         ...
         <label for="nf-name">Mail id</label>
         <input class="form-control {{ $errors->has('mail_id') ? ' is-invalid' : '' }}" autocomplete="random" value="@if($updatedata!=''){{$updatedata->mail_id}}@endif"  id="nf-mail_id" name="mail_id" type="email" 
               placeholder="Enter mail id.." autocomplete="mail_id">
         @if ($errors->has('mail_id'))
                    <span class="invalid feedback" role="alert"> 
                        <strong class="text-danger">{{ $errors->first('mail_id') }}.</strong> 
                    </span>
            @endif </div>
                     ...
                     <tbody>
                        ...
                        @foreach($data as $value)
                           ...
                           <td class="action ">
                              ...
                              <a class="icon-btn edit" href="{{ route('settings.contactussettings.email',$value->id) }}"><i class="fal fa-edit" id="show-edit"></i></a>
                            <a class="icon-btn delete universaldelete" href="javascript:void(0);" data-status="0" data-id="{{ $value->id}}"  data-model="Email" id="{{$value->id}}" > <i class="fal fa-trash-alt" id="delete-btn"></i></a> 
                           </td>
                        </tr>
                        @endforeach
                     </tbody>
